...
public class TrajectoryPIDController implements LiveWindowSendable {
   ...
   *        default is 50ms.
   */
  public TrajectoryPIDController(double Kp, double Ki, double Kd, PIDSource source, PIDOutput output,
      double period) {
...
/**
  ...
  protected double calculateTurn() {
	...
	double angError = (m_setpoint_ang * (180.0 / Math.PI)) - (m_Gyro.pidGet());
   ...
   * heading versus the requested heading. m_turnDirection is used to set the
   * polarity on the returned value (add to one robot side, subtract from the other
   * in order to cause the robot to turn). This method as apposed to the above corrects for turn
   ...
   */
  public void setSetpoint(TrajectorySegment s){
